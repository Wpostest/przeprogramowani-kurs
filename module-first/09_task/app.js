  ...
  */
  ...
  function getModulesCount(){
    ...
    lessons.forEach(item => {
      const moduleNumber = (item.name[1] * 1);
  ...
  function totalDuration() {
    let totalHours = 0;
    let totalMinutes = 0;
    const modulesTime =  [...Array(getModulesCount())].map(e => new Array(2).fill(0));
  
    lessons.forEach((item, index) => {
      const arrayTime = item.length.split(":");
      totalHours += parseInt(arrayTime[0]);
      totalMinutes += parseInt(arrayTime[1]);
      const courseNumber = (item.name[1] * 1);
  
      if(Number.isInteger(courseNumber)){
        //modulesTime[module index][0 - hours, 1 - minutes] += hour | minutes  
        modulesTime[courseNumber - 1][0] += parseInt(arrayTime[0]);
        modulesTime[courseNumber - 1][1] += parseInt(arrayTime[1]);
      }
    });
    ...
    //MODULE 1
    const hours1 = Math.floor(modulesTime[0][0] + modulesTime[0][1] / 60);
    const minutes1 = modulesTime[0][1] % 60;
    //MODULE 2
    const hours2 = Math.floor(modulesTime[1][0] + modulesTime[1][1] / 60);
    const minutes2 = modulesTime[1][1] % 60;
    //MODULE 3
    const hours3 = Math.floor(modulesTime[2][0] + modulesTime[2][1] / 60);
    const minutes3 = modulesTime[2][1] % 60; 
  
    //PERCENTAGE
    const courseTime = hours * 60 + minutes;
    const moduleFirstPercentage = Math.round((hours1 * 60 + minutes1) * 100 / courseTime);
    const moduleSecoundPercentage = Math.round((hours2 * 60 + minutes2) * 100 / courseTime);
    const moduleThirdPercentage = Math.round((hours3 * 60 + minutes3) * 100 / courseTime); 
    return `
    Kurs Opanuj JavaScript trwa ${hours} godzin i ${minutes} minut.
    Moduł Adama: ${hours1} godzin ${minutes1} minut jest to ${moduleFirstPercentage}% kursu
    Moduł Przemka: ${hours2} godzin ${minutes2} minut jest to ${moduleSecoundPercentage}% kursu
    Moduł Marcina: ${hours3} godzin ${minutes3} minut jest to ${moduleThirdPercentage}% kursu`;
  }
